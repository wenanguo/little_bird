...
class PlayerCreator {
    constructor() {
        ...
        this.song_name_list = $('.music__list_content_name');
    ...
    init() {
        ...
        this.renderSongListName();
    }
    //生成播放列表
    renderSongList() {
        let _str = '';
        this.musics.songs.forEach((song, i) => {
            _str += `<div class="music__list__item"></div>`;
        });
    ...
    renderSongListName() {
        let _str = '';
        this.musics.songs.forEach((song, i) => {
            _str += `<div class="music__list__item"><h1>${song.title}</h1><h2>${song.singer}</h2></div>`;
        });
        this.song_name_list.html(_str);
    }
    //根据歌曲去渲染视图
    renderSongStyle() {
        let {
